...
ProcessSelectionDialog::ProcessSelectionDialog(QWidget *parent)
    : QDialog(parent)
{
    this->setWindowTitle(trXml("dlgPickWinTitle", "Select Running Process to Protect"));
    this->resize(580, 520);
    ...
    m_txtSearch->setPlaceholderText(trXml("dlgPickSearchPlaceholder", "Search running process by name or PID..."));
    connect(m_txtSearch, &QLineEdit::textChanged, this, &ProcessSelectionDialog::filterProcesses);
    searchLayout->addWidget(m_txtSearch);

    QPushButton *btnRefresh = new QPushButton(trXml("btnRefreshProcesses", "Refresh"), this);
    ...
    QStringList headers;
    headers << trXml("colIcon", "Icon")
            << trXml("colProcName", "Process Name")
            << trXml("colPid", "PID")
    ...
    m_btnSelect = new QPushButton(trXml("btnProtectSelectedProc", "Protect Selected Process"), this);
    ...
    m_btnCancel = new QPushButton(trXml("btnCancel", "Cancel"), this);
...
void ProcessSelectionDialog::onItemSelected() {
    ...
    if (row < 0) {
        QMessageBox::warning(this, trXml("dlgPickNoSelectionTitle", "No Selection"), trXml("dlgPickNoSelectionText", "Please select a running process from the list."));
        return;
    }
