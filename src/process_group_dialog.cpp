...
ProcessGroupInfoDialog::ProcessGroupInfoDialog(const QString &groupName, const QList<DWORD> &pids, QWidget *parent)
    ...
    , m_pids(pids)
{
    this->setWindowTitle(QString("Process Group Diagnostics - %1 (%2 Instances)").arg(groupName).arg(pids.size()));
    this->resize(720, 540);
    ...
    QGroupBox *groupCard = new QGroupBox("Process Group Identity", this);
    ...
    m_lblTitle = new QLabel(QString("<b>%1</b> (Group of %2 Active Processes)").arg(groupName).arg(pids.size()), groupCard);
    m_lblTitle->setStyleSheet("font-size: 11pt; color: #38bdf8;");

    m_lblSummary = new QLabel("Calculating total group metrics...", groupCard);
    ...
    QGroupBox *groupInstances = new QGroupBox("Active Process Instances && Windows Services", this);
    ...
    QStringList headers;
    headers << "PID" << "Windows Service / Description" << "RAM Working Set" << "Open Handles";
    ...
    m_btnProtect = new QPushButton("Protect Group", this);
    ...
    connect(m_btnProtect, &QPushButton::clicked, this, [this]() {
        ...
        QMessageBox::information(this, "Group Protected", QString("Process group '%1' added to exclusion list.").arg(m_groupName));
    });

    m_btnTrim = new QPushButton("Trim Group RAM", this);
    ...
    m_btnKill = new QPushButton("Task Kill Group", this);
...
void ProcessGroupInfoDialog::refreshGroupDiagnostics() {
    ...
    for (DWORD pid : m_pids) {
        ...
        if (serviceDesc.isEmpty()) {
            serviceDesc = "Standard Executable Process";
        }
    ...
    m_lblSummary->setText(QString("Total Combined RAM: <b>%1 MB</b> | Avg per Instance: <b>%2 MB</b> | Total Open Handles: <b>%3</b>")
