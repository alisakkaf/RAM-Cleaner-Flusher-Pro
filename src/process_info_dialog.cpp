...
void ProcessInfoDialog::setupUI() {
    this->setWindowTitle(QString("Process Inspector - %1 (PID: %2)").arg(m_procName).arg(m_pid));
    ...
    // Header Card (Compact & Centered Left)
    QGroupBox *headerGroup = new QGroupBox("Process Identity", this);
    ...
    m_lblPid = new QLabel(QString("Process ID (PID): %1").arg(m_pid), this);
    ...
    // Diagnostics Group
    QGroupBox *detailsGroup = new QGroupBox("Real-Time Diagnostics & System Info", this);
    QVBoxLayout *detailsLayout = new QVBoxLayout(detailsGroup);
    detailsLayout->setSpacing(6);
    detailsLayout->setContentsMargins(12, 12, 12, 12);

    m_lblPath = new QLabel("Executable Path: Resolving...", this);
    m_lblPath->setWordWrap(true);

    m_lblWorkingSet = new QLabel("RAM Working Set: 0 MB", this);
    m_lblPeakWorkingSet = new QLabel("Peak RAM Usage: 0 MB", this);
    m_lblPagefile = new QLabel("Pagefile Commitment: 0 MB", this);
    m_lblPriority = new QLabel("CPU Priority Class: Normal", this);
    m_lblHandles = new QLabel("Open Handles Count: 0", this);
    m_lblStartTime = new QLabel("Start Time: Unknown", this);
    ...
    // Actions Bar (Clean Buttons, NO double emojis!)
    QHBoxLayout *actionsLayout = new QHBoxLayout();
    actionsLayout->setSpacing(8);

    m_btnProtect = new QPushButton("Protect Process", this);
    ...
    m_btnTrim = new QPushButton("Trim Working Set", this);
    ...
    m_btnKill = new QPushButton("Task Kill", this);
...
void ProcessInfoDialog::refreshProcessDetails() {
    ...
    if (!hProc) {
        m_lblWorkingSet->setText("RAM Working Set: Process Exited / Access Denied");
        return;
    }
    ...
    if (GetModuleFileNameExW(hProc, NULL, pathBuf, MAX_PATH) > 0) {
        m_lblPath->setText(QString("Executable Path: %1").arg(QString::fromWCharArray(pathBuf)));
    } else {
        m_lblPath->setText(QString("Executable Path: %1").arg(m_procName));
    }
    ...
    if (GetProcessMemoryInfo(hProc, &pmc, sizeof(pmc))) {
        ...
        m_lblWorkingSet->setText(QString("RAM Working Set: %1 MB (%2 bytes)").arg(wsMB, 0, 'f', 2).arg(pmc.WorkingSetSize));
        m_lblPeakWorkingSet->setText(QString("Peak RAM Usage: %1 MB").arg(peakMB, 0, 'f', 2));
        m_lblPagefile->setText(QString("Pagefile Commitment: %1 MB").arg(pageMB, 0, 'f', 2));
    }

    // 3. Priority Class
    DWORD prio = GetPriorityClass(hProc);
    QString prioStr = "Normal";
    if (prio == HIGH_PRIORITY_CLASS) prioStr = "High Priority";
    else if (prio == ABOVE_NORMAL_PRIORITY_CLASS) prioStr = "Above Normal";
    else if (prio == BELOW_NORMAL_PRIORITY_CLASS) prioStr = "Below Normal";
    else if (prio == IDLE_PRIORITY_CLASS) prioStr = "Idle";
    else if (prio == REALTIME_PRIORITY_CLASS) prioStr = "Realtime";

    m_lblPriority->setText(QString("CPU Priority Class: %1").arg(prioStr));

    // 4. Handle Count
    DWORD handleCount = 0;
    if (GetProcessHandleCount(hProc, &handleCount)) {
        m_lblHandles->setText(QString("Open Handles Count: %1").arg(handleCount));
    }
    ...
    if (GetProcessTimes(hProc, &ftCreate, &ftExit, &ftKernel, &ftUser)) {
        ...
        m_lblStartTime->setText(QString("Process Start Time: %1").arg(dt.toString("yyyy-MM-dd hh:mm:ss AP")));
    }
